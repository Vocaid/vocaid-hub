...
**Architecture:** World (Trust) + 0G (Verify) + Arc (Trade)
...
**Evolution:** v3 Hybrid Resource Exchange → v4 Hybrid Resource Allocation (World+0G+Arc, no Hedera)
...
| Arc RPC | ✅ Verified | chainId `0x4cef52` (5042002) responding |
...
| ~~Arc testnet USDC~~ | ~~faucet.circle.com~~ | ✅ Funded `0x58c456...` |
...
| `TECHNOLOGY_RESEARCH.md` | ERC-8004, OpenClaw, 0G, Arc, x402, Automata DCAP |
| `DEVELOPER_CONVERSATIONS.md` | 0G gap storytelling, Arc pitch, DCAP talking points |
| `OPENCLAW_RISK_ASSESSMENT.md` | Security risks, gaps, mitigations, Go/No-Go |
| `PRE_HACKATHON_CHECKLIST.md` | Updated for World+0G+Arc stack |
...
|-----------|----------|----------|
| Payment chain | Hedera (HBAR) | Arc (USDC Nanopayments) |
| Token standard | HTS non-transferable | ERC-8004 Identity NFTs |
| Audit trail | HCS (Hedera Consensus) | ERC-8004 Reputation Registry |
| Agent identity | HCS-10 | ERC-8004 + World AgentKit |
| Settlement | x402 HBAR (no facilitator) | x402 USDC via Circle Gateway |
| Prediction markets | None | ResourcePrediction.sol on Arc |
| GPU verification | None | GPUProviderRegistry + Automata DCAP ZK |
| Credential tokens | 5 HTS tokens | ERC-8004 metadata entries |
| Partner 3 | Hedera ($15k) | Arc ($15k) |
...
| `DEVELOPER_CONVERSATIONS.md` | Hour 0 (sponsor meetings) | Talking points for 0G, Arc, World booths |
...
|-------------|-------------|------|-------|-------------|
| `skills/nanopayments.md` | Agent 9 | W3 | ~30 | Wraps `@circle-fin/x402-batching` for agent USDC payments |
...
| `viem` | ^2.0.0 | Contract interaction | Arc | Circle SDK uses viem internally |
...
| `@circle-fin/x402-batching` | ^1.0.0 | Nanopayments | Arc | Official Circle SDK. Pin version, not "latest" |
| `@x402/fetch` | ^2.0.0 | x402 protocol | Arc | Coinbase standard |
...
| **Arc Agentic Nanopayments** | $6k | W3 | Agent-to-agent USDC via Circle Nanopayments |
| **Arc Prediction Markets** | $3k | W3 | Resource pricing markets (GPU, skills) in USDC |
...
| **3: Arc Contracts** | `fin/wave1-arc` | Deploy ResourcePrediction.sol on Arc Testnet (USDC at `0x360...`). Deploy NanopaymentRouter.sol. Set up Circle Nanopayments buyer/seller SDK. Record addresses in `deployments/arc-testnet.json`. | Arc |
...
**Merge order:** Agent 4 (Scaffold) → Agent 1 (0G) → Agent 2 (World) → Agent 3 (Arc)
...
- [ ] ResourcePrediction deployed on Arc Testnet (`testnet.arcscan.app`)
...
| Agent | Branch | Deliverable |
|-------|--------|-------------|
| **9: Circle Nanopayments** | `fin/wave3-nanopayments` | Circle Nanopayments integration: buyer deposits USDC → Gateway Wallet. x402 middleware on resource API endpoints (402 response without payment). Agent-to-agent USDC payments (gas-free via EIP-3009). Payment confirmation UI. |
...
- [ ] Agent pays $0.01 USDC for GPU inference via Nanopayments (gas-free)
...
| **14: Submission Assets** | `fin/wave4-submission` | README.md with architecture diagram + setup instructions. Demo video (<3 min for 0G, <5 min for World/Arc). SUBMISSION.md with contract addresses + team info. AI_ATTRIBUTION.md. |
...
- [ ] Video recorded: <3 min (0G), <5 min (World/Arc)
...
```text
Hour 0-1:    Sponsor conversations (0G, Arc, World) — HIGHEST ROI
...
| 3 | W1 | `fin/wave1-arc` | ResourcePrediction + Nanopayments on Arc | Arc Prediction, Nano |
...
| 9 | W3 | `fin/wave3-nanopayments` | Circle Nanopayments (USDC) | Arc Agentic Nano |
| 10 | W3 | `fin/wave3-predictions` | Prediction market UI + seeded markets | Arc Prediction Markets |
...
Hour 0: Phase 0 — Repository Init
  ...
  #2  init: add .env.example with World + 0G + Arc variables
  #3  init: add multi-chain Hardhat config (0G Galileo + Arc + World Sepolia)
...
Hour 1-2: Wave 1 Agent 4 — OpenClaw + Dependencies
  ...
  #8  chore: install project dependencies (ethers, viem, 0G SDK, Circle SDK)
...
Hour 2-4: Wave 1 Agent 3 — Arc Contracts
  #16 feat(arc): add ResourcePrediction.sol (USDC prediction market)
  #17 feat(arc): deploy ResourcePrediction on Arc testnet
  #18 feat(arc): add Circle Nanopayments buyer/seller setup
  #19 feat(arc): record all deployment addresses
...
Hour 10-12: Wave 3 Agent 9 — Nanopayments
  #32 feat(arc): add Circle Nanopayments payment flow
  #33 feat(arc): add x402 middleware for paid resource queries
...
| **Arc track** | <5 min | Nanopayments + prediction markets (steps 5, 6) |
...
| 5 | 30s | Create prediction market: "Will H100 cost drop?" → place USDC bet | Arc Prediction | `/predictions` → create + bet |
| 6 | 30s | Agent pays $0.01 USDC for GPU inference → gas-free | Arc Nanopayments | `/` → hire resource → payment confirmation |
...
- "Agent pays $0.000001 USDC — zero gas — sub-second settlement"
...
| Arc testnet down | Show prediction market contract on `testnet.arcscan.app` + explain x402 flow with diagram |
...
> - Account setup (World Developer Portal, 0G wallet, Arc testnet, Bonsai API)
...
> - ResourcePrediction.sol on Arc + prediction market UI
> - Circle Nanopayments integration for agent-to-agent USDC payments
...
> - @worldcoin/minikit-js, @0glabs/0g-serving-broker, @circle-fin/x402-batching
