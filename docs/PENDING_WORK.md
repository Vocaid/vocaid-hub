...
| P-081 | Retroactive reputation: on-chain writes need 3rd wallet | ✅ done | — | `scripts/compute-retroactive-reputation.ts` | Fixed: separate REPUTATION_WRITER_KEY (0x6809...E829) for Phase 5. Phase 4 registers identities only (no GPUProviderRegistry 1-per-wallet limit). Full run: 8 providers, 371 txs, 56 on-chain reputation writes (7/7 per provider), HCS audit logged. |
